...
| 25 | **A mid-turn hang survives BOTH liveness guards (flat config)** | 2026-07-17 arm D (`review_mode/plan_approval/qa=off`, fork on): worker rewrote `todo.py` + 4 test files (37 tests pass) on `main`, uncommitted, then the agent loop **hung** — GPU 0% for 20+ min, no daemon logs, no inference calls, inference plane healthy. **Neither guard recovered it:** the flail guard's read-without-edit counter was reset by the 16:04 edit (edit-then-hang); the stall watchdog (900s) skips the effort because the worker still reports `computing` (busy≠stalled). The hang lives in the gap between them. **`review_mode=off` made it worse** — arms A/B's checkpoint cycles double as liveness heartbeats; flat config emits none, so the hang is invisible until a coarse watchdog that then doesn't fire. | **FIXED + DEPLOYED** `f19fe00` (2026-07-17) — silence detector, bridge-only (no daemon rebuild: the daemon already exposes `/tasks/{id}/events` `next_offset`). `worker_silence_s=300`; a running worker whose event offset is frozen past it → cancel turn + recover. Immune to the `GET /health` false heartbeat and the edit-phase (`activity=0`, GPU 100%) false positive; fail-safe never kills an unobservable-but-running daemon. Repro-first, 531 green, live-verified (`next_offset=13256` on a real turn). **Live-validated in gym-006 (PR#14): dormant + correct across 22 completed worker turns, `stall_recovered=0` — zero false-positive kills on a healthy full-product round.** (A real hang is nondeterministic, so the recovery path is proven by the unit repro, not yet live.) See "The worker execution loop → Liveness". |
| 26 | **The test-scope monitor false-positive-froze a healthy effort** | 2026-07-17 gym-007 (corpus-proof round): the PM monitor froze the effort (steering, trigger `deviation`) on *"fewer tests executed than expected."* Investigated live: the worker's test change was **purely additive** — template baseline `test_todo.py` = 5 test functions → worker's = 48 (+361/-0), all 48 pass cleanly, nothing deleted or weakened. A **false positive** that cost a human-in-the-loop decision the dark factory is supposed to avoid — the exact O(n)-human-attention tax. The heuristic ("fewer than expected") has no stable baseline: a worker that reorganizes or runs a subset mid-iteration trips it. | **OPEN** — the monitor needs a *deterministic* baseline (compare test count to the BASE commit's, flag only a genuine DROP), not an LLM "expected" guess. Relates to P9 #3 (deterministic risk). Cleared by operator-role investigation + `operator_decision` note. |
