...
class Pipe:
    class Valves(BaseModel):
        ...
        TIMEOUT_SEC: int = Field(
            default=120, description="Timeout for script execution"
        )
        ENABLE_GPU_CHECK: bool = Field(
            default=True, description="Check GPU availability on startup"
        )
        LOG_EXECUTION: bool = Field(
            default=True, description="Enable execution logging"
        )
    ...
    def _format_response(self, result: Any) -> str:
        ...
        if isinstance(result, dict):
            ...
            # Router status response (for compatibility with router)
                ...
                result.get("service") == "AI Stack Unified Router"
                and "loaded_modules" in result
            ):
    ...
    def pipe(self, body: dict, __user__: dict) -> str:
        ...
        try:
            ...
            payload = {
                ...
                or "2024-01-01T12:00:00Z",  # Provide default timestamp
