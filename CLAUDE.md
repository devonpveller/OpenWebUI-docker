...
| **Open Brain** (`open-brain`) | `docker compose -f OB1/docker/docker-compose.yml ...` | **30 containers** (the render's own count with all four profiles; 29 against the PINNED gitlink 5005197, which declares only `idea-refinery` — `stack.py inventory --check` reports the other three as `[declared, not rendered]` until the gitlink bumps): the `openbrain-*` fleet + its two backup sidecars + the **Open Notebook trio** (`surrealdb`, `open_notebook`, `open-notebook-backup` — moved in K.5b 2026-08-21; ON stays live until the wiki workbench matures). Attaches to `ai-stack_llm-net`/`app-net` externally. Bring up **after** `llm-gateway` is healthy; tear down before the planes it depends on. |
