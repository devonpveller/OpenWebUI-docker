...
| **Open Brain** (`open-brain`) | `docker compose -f OB1/docker/docker-compose.yml ...` | **30 containers**, both numbers measured against the PINNED gitlink 5005197: a bare render gives 29 and `--profile idea-refinery` gives 30, because that is the only profile the pinned commit declares. `research`/`wiki`/`notebook` exist in the manifest and not yet in the submodule, so they gate nothing today — `stack.py inventory --check` reports them as `[declared, not rendered]` until the gitlink bumps. The thirty are: the `openbrain-*` fleet + its two backup sidecars + the **Open Notebook trio** (`surrealdb`, `open_notebook`, `open-notebook-backup` — moved in K.5b 2026-08-21; ON stays live until the wiki workbench matures). Attaches to `ai-stack_llm-net`/`app-net` externally. Bring up **after** `llm-gateway` is healthy; tear down before the planes it depends on. |
