...
| 5 | **`stack.py health`** | If the service has a meaningful probe (host port or exec-able health endpoint), add a probe to `HealthSweep.run()` in `scripts/stack/stack.py` **and its name to `PS1_PROBES` in `scripts/stack/test_stack.py`** (the list is pinned, so a probe cannot be added or dropped unnoticed). `stack.ps1 health` is a shim over that sweep since 2026-09-19. Uptime is watched remotely — probes are how absence gets noticed. **THE INFERENCE LANDING CHECK lives here: no inference recreate is "healthy" until a completion has returned 200.** The `inference: serving depth` probe counts `.gguf` files under `llama-cpp-upstream:/models` (zero is a FAIL naming the HOST bind), then reads llama-swap `/running`, and only when nothing is resident spends ONE 3-token completion THROUGH the gateway — a cold load is minutes, 257 s measured 2026-09-21, so its timeout is generous. It exists because the other fifteen probes — this is the sixteenth — were all green for thirty hours (2026-09-19 18:54 → 09-21 00:57) while every chat returned `500 upstream command exited prematurely`: the upstream had been recreated with `LM_MODELS_DIR` unset, compose bound its empty default at `/models`, and llama-swap's `/health` answers without loading a model. **After recreating any inference container, run `python scripts/stack/stack.py health` and read that line** — container-healthy is not serving. |
...
| 7 | **Backups** | Stateful data ⇒ a backup sidecar IN THE SAME PROJECT (runbooks/backup-conventions.md): script in `backup/` (or the project's own `backup/` dir for submodules), sleep-loop for interval tars / supercronic for cron-timed dumps, sha256 sentinel, output under `./backups/<name>/`. Then: `scripts/sysadmin-mcp/check_backups.py` `_EXPECTED` row, `scripts/checks/check-backup-coverage.ps1` volume map, and a restore entry in BOTH `scripts/backup/restore-from-snapshot.ps1` (catalog) and `runbooks/restore-from-snapshot.md`. **TWO GATES, and they answer different questions — a service is not backed up until both pass.** (1) `scripts/checks/check-backup-coverage.ps1` — COVERAGE: is every volume either mapped to a sidecar or in `$intentionallyExcluded` with a reason? (2) `scripts/checks/check-backup-freshness.ps1` (over `scripts/sysadmin-mcp/check_backups.py --check`) — PRODUCTION: did that sidecar actually emit an artifact inside its cadence, and is it not currently declining runs? Coverage passed for the whole 350 h that `lm-models-backup` answered every run with `PRECHECK SKIP: /data is empty` — the sidecar existed and referenced the volume, which is all coverage ever asked. Freshness reads the sidecar's own log and FAILS on a mount that is empty or absent, naming it, because that is a bind pointing where the data is not and it never heals itself. Neither gate is pre-commit: both read `./backups/`, which is host state. |
...
| 8a | **Profile?** | If the service sits behind a compose `profiles:` key, say so in FOUR places or it becomes invisible. (1) `scripts/lib/stack-services.curated.json`: the row's `profile` field + the project's `profiles` list, then `python scripts/stack/stack.py inventory --write` (`scripts/lib/stack-services.json` is GENERATED - see row 8). Where the plane's compose file is a PINNED SUBMODULE whose commit does not carry the profile yet, the curated `profile` is a DECLARATION: `inventory --check` prints it as `[declared, not rendered]` and verifies it once the gitlink bumps. NO row is in that state today - `sl-ob1-gitlink` bumped OB1 `5005197` -> `fe3e045` on 2026-09-20 and all ten profiled open-brain rows are derived from the render again. (2) `stack.manifest.toml`: the `[planes.<plane>.profiles.<name>]` table, the product that pulls it (`profiles` for an engine the product needs, `surfaces` for something `--headless` may drop), and `requires` if the profile's service calls an engine that lives in ANOTHER profile — otherwise it comes up healthy and silently produces nothing. (3) `scripts/checks/check-project-configs.ps1`: the render target must pass the profile, or the coverage assertion fails the commit (it compares rendered containers against ALL inventory rows for the project, deliberately not against the profiles the target passes — deriving the expectation from the argument list makes the guard police itself). (4) **Who reaches it from another project**: `portal/config/caddy/Caddyfile`, `status-pipe/modules/`, `frontend/entrypoint.sh` routes. Profiled planes today, FIVE: `frontend` (`stock`, `gpu`, `tailscale`), `inference` (`local`), `agent-org` (`workers`, `cloud`), `portal` (`internet`), `ob1` (`research`, `wiki`, `notebook`, `idea-refinery` - ALL FOUR in the pinned gitlink since `fe3e045`, and a bare OB1 render is 20 services against 30 with all four). **Where OB1's four are declared, per D17, and the LANDING STEP for this host** (measured 2026-09-20; both are needed, and they do not fight): **(i) `COMPOSE_PROFILES=research,wiki,notebook,idea-refinery` in `OB1/docker/.env`** - the per-plane env file. Read 2026-09-20: `frontend/.env` carries `gpu,tailscale`, `inference/.env` carries `local`, `OB1/docker/.env` carries nothing (`portal` deliberately has none and `agent-org`'s slices are operator-driven), and that absence is what makes a BARE `docker compose -f OB1/docker/docker-compose.yml stop|down|up` address 20 of the 30. **A bare `down` is the dangerous one**: it removes the unprofiled 20, leaves the ten gated ones running, and then fails to drop the project network with `Resource is still in use` - measured in a throwaway two-service project, and seven of OB1's ten gated containers hold endpoints on `ai-stack_llm-net`/`app-net`/`default`, which is exactly what `emergency-recovery.ps1`'s teardown order exists to free. **(ii) the DRIVER'S STATE** - `python scripts/stack/stack.py enable research` writes `idea-refinery, research, wiki, notebook` into `.stack/state.json`, and `up --dry-run` then prints all four `--profile` flags on the OB1 line. **`enable`, not `init --product research --force`**: this host HAS a state file, and `init --force` builds a fresh one rather than merging - measured 2026-09-20, a six-plane state (`frontend, inference, memory, search, coder, agent-org`) came back as four (`frontend, inference, ob1, search`) with `memory`, `coder` and `agent-org` dropped, while `enable research` on the same input gave seven. Both print the same `enabled product research:` summary, so the screen does not warn you. See `scripts/stack/README.md`, the `init` section. **Say the consequence out loud**: a CLI `--profile` REPLACES `COMPOSE_PROFILES` rather than adding to it, and the driver UNIONS the plane env's list into its own flags (`effective_profiles()`), so on a host whose `OB1/docker/.env` carries all four, **`--headless` becomes a no-op for this plane** - it drops `wiki`/`notebook` from the driver's set and the env puts them straight back. That is the price of (i), and it is the right trade on a host that runs all 30; a host that genuinely wants a headless OB1 must leave (i) out and use (ii) alone. Neither is set on this host yet: `stack.py up ob1` therefore passes `idea-refinery` + `research` (the `default` plus its `requires` closure), which RENDERS 23 - seven short of the 30 that are running. **Render the set, never add up the deltas**: 22 is `--profile research` alone, and attempt 1 of this item put that number in eight files by arithmetic. |
...
(`idea-refinery`, `research`, `wiki`, `notebook` - all four carried by the
pinned gitlink since `fe3e045`, 2026-09-20). A profiled service is
...
```powershell
python scripts/stack/stack.py health           # 16 functional probes, all planes; exit code = failures
