...
| 8a | **Profile?** | If the service sits behind a compose `profiles:` key, say so in THREE places or it becomes invisible: the row's `profile` field + the project's `profiles` list in `scripts/lib/stack-services.json`; the `[planes.<plane>.profiles.<name>]` table in `stack.manifest.toml` (and the product that should pull it — `profiles` for an engine the product needs, `surfaces` for something `--headless` may drop); and the render target in `scripts/checks/check-project-configs.ps1`, which must pass every profile or its row diff silently stops verifying the gated containers instead of failing. Profiled planes today: `inference` (`local`), `agent-org` (`workers`, `cloud`), `portal` (`internet`), `ob1` (`research`, `wiki`, `notebook`, `idea-refinery`). |
