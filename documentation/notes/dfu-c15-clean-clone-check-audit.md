...
`quadrant.prove_guards` 25/25 bite, `pytest scripts/agent-harness` 298 passed, `ruff` clean.
...
`ALL MUTATIONS RED - every guard can fail`, while all six "RED" lines carry
`E   ModuleNotFoundError: No module named 'sqlalchemy'`. It decides a mutation was caught from
`returncode != 0` and never asserts that the UNMUTATED tree is green, so any interpreter that
cannot import the package under test satisfies every mutation at once. It was one of the two
...
`git clone` does not help: clone creates a local branch only for the remote's HEAD, and
`git rev-parse` does not resolve a bare name through `refs/remotes/origin/`. The base is
hard-coded with no parameter, so no INVOCATION fixes it. Harness owner.
...
**F6. `scripts/checks/test-quartz4-offline.ps1 -Phase unit` is red on the work line.** Exit 1,
...
constraint`). The harness's fixture SQL was not updated when the constraint landed. It is
therefore not U1's marker - `smoke-agent-memory.ps1` is, and it is green - but somebody owns
this.
...
real drills: U8's is roughly 15 minutes and U1's and U5's build throwaway containers. A full
`dfu-done.ps1` now spends roughly an hour in its sandbox. That is what re-running the columns
...
| U1 | `scripts/checks/smoke-agent-memory.ps1` | exit 0, 23 checks, `ALL AGENT-MEMORY SMOKE CHECKS PASSED` |
...
| U5 | `scripts/checks/drill-personal-plane-exclusion.ps1 -AcceptDispositionedGaps` | exit 0, `CONTAINMENT GREEN, 25 gap(s), ALL DISPOSITIONED (106 checks passed, 0 failed)` |
| U8 | `scripts/checks/verify-dfu-done.ps1 -Target scripts/checks/dfu-done.ps1` | exit 0, `DRILL GREEN - 216 assertions, 0 failed` |
