...
| 2026-05-17 | D23 | **Compile schedule = deterministic daily + change-driven (replaces 24h-from-boot interval).** User disliked the boot-relative 24h timer (drifts, can fire mid-work). Now: (a) `WIKI_RECOMPILE_HOUR` local-time daily (default 01:00; set service `TZ`); (b) change-watcher polls OpenBrain every `WIKI_WATCH_INTERVAL_MIN` (3m) for sources/thoughts newer than last compile and compiles once activity *settles* (debounced burst coalescing — decoupled from deep_research, covers all source producers); (c) boot; (d) on-demand. Verified: scheduler logs `next daily ~01:00`; change-watch active. | User: predictable 1am + event-driven after research, no unpredictable interval |
