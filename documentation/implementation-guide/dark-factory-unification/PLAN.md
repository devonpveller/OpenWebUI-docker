...
| **U4** | **Runner unification:** prove `little-coder` through one real anchored item end to end (the standing unproven claim, A11); then agent-org workers as harness runners and vice versa; frontier-oracle-on-stall wired per §7. **Amended 2026-08-30 (work/u4bidir, class 2):** the original clause read "one *profile* mechanism governs both" and that is **FALSE** — agent-org's profile binds a role to a model LANE for the bridge's own inference calls and has never governed a worker, while the harness's binds a role to a RUNNER; a merged table would force each side to carry fields it cannot use. The unifiable object is one layer down and is what shipped: the **RUNNER REGISTRY** (what substrates exist, of what kind, at what address), read by `config.ps1`, `config.py` and `agent-bridge/app/modules/runners.py`. The two directions are also not equally done: agent-org **dispatches** on it (`RunnerDispatch` on the live wake path); the harness **declares** it and has no dispatcher, so that half is parked with the park asserted by test. Evidence and revert paths: `documentation/notes/u4bidir-findings.md`. | Gym: same anchored item run per quadrant (runner × target), outcomes compared; stall→oracle observed firing at least once | U2 |
