...
| A11 | 95% small-model search / 5% frontier oracle-on-stall (§7) | agent-org | **PARTIALLY PROVEN 2026-08-30 (was UNTESTED).** "Wired" was optimistic: `Resolve-RoleTarget` resolved a runner and *nothing dispatched to one* — the config named a door (`127.0.0.1:8090`) the compose file never published. `dispatch.ps1` is that missing half; one real item then completed on the local runner (qwen36-27b), verified by an acceptance command the daemon ran itself — task `01M19JABFNQHR7CPDGCPDK2VEV`, commit `247bac7`, host suite 3 failed→18 passed. **Still untested: the 95/5 RATIO, the oracle-on-stall path (nothing escalates a `status: timeout`), and delivery** — the local runner cannot push to this repo (403) so the artifact came back by `git bundle`, by hand. `status: "unproven"` is deliberately still in the config. Evidence + parked list: `documentation/notes/dfu-u4-findings.md`. |
...
| **U4 status (2026-08-30)** | **PARTIALLY DONE, REMAINDER PARKED.** The first clause is satisfied: `dispatch.ps1` + `verify-dispatch.ps1` (33/33 incl. a live probe) and one real item completed on the local runner (see A11). | The *Validated by* column is NOT satisfied — no quadrant comparison was run and no oracle-on-stall path exists. Blocking finding: the local runner cannot push to this repo (`LC_DEPLOY_TOKEN` → 403), so every local-runner item needs a delivery path the harness does not own yet (`documentation/notes/dfu-u4-findings.md` finding 4). U3's seeded-regression gym run, parked here by the 2026-08-30 DECISIONS entry, therefore stays parked. | — |
