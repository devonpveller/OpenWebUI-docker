...
- [ ] **0.1** Working branches (no commits): `ai-stack` → `feature/integrated-knowledge-system`; `OB1` → local branch; `open-notebook` → local branch
- [ ] **0.2** `iks-dev/docker-compose.dev.yml` — project `iks-dev`: `iks-db`, `iks-mcp`, `iks-surreal`, `iks-notebook` (fresh volumes, non-colliding ports, no prod volume/container)
- [ ] **0.3** `iks-dev/seed.sql` — ~3 threads, ~15 sources (some overlapping), a couple of sessions (synthetic only)
- [ ] **0.4** `iks-dev/README.md` — up/down, ports, "never touches prod" warning
- [ ] **0.5** `iks-dev/baseline-inventory.md` — snapshot via `/stack-map`
...
- [ ] **1.1** `OB1/docker/init-threads.sql` (runs after `init-sources.sql`): `threads`, `thread_sources` (link_type ∈ automatic/suggested/deliberate, status ∈ confirmed/pending/hidden/inactive), `sessions`, `session_sources` + touch-triggers + indexes
- [ ] **1.2** `ALTER TABLE sources ADD COLUMN IF NOT EXISTS content_hash TEXT` + index
- [ ] **1.3** `find_or_create_source(...)` — match on `url` OR `content_hash`; returns `(id, was_duplicate)` ⚠ *also unblocks 3.0*
- [ ] **1.4** Lifecycle fns: `link_source_to_thread`, `set_thread_source_status` (upsert/flag, never delete)
- [ ] **1.5** Mirror as `OB1/schemas/research-threads/` contrib *(optional)*
...
- [ ] **2.1** 11 tools in `kubernetes-deployment/index.ts` (mirror existing registration + `x-brain-key`): `create_thread`, `list_threads`, `get_thread_sources`, `add_to_thread`, `remove_from_thread`, `get_suggestions`, `accept_suggestion`, `hide_suggestion`, `get_hidden_suggestions`, `restore_suggestion`, `capture_with_thread`
- [ ] **2.2** Gateway stays closed — do **not** add to `openbrain-gateway/app.py` allow-list; add code comment + runbook note (guardrail 5)
- [ ] **2.3** OpenAPI exposure — **no mcpo config entry needed (C3):** restart `openbrain-mcpo` (core, ≠ `openbrain-mcpo-ext`) so it re-reads `tools/list`, re-import in OWUI
...
- [ ] **3.0 ⚠ PREREQUISITE (C1)** — refactor `/research/persist` per-source replace from hard-`DELETE`+INSERT to `find_or_create_source` (stable `id`s, dedup-and-relink) so thread/session FKs survive re-runs
- [ ] **3.1** Extend payload (`evidence_memory.py::persist_research_evidence` ~L188) + handler (`index.ts` ~L853): create `sessions` row, `session_sources`, `thread_sources(automatic,confirmed)` if `thread_id`, stamp provenance into `sources.metadata`
- [ ] **3.2** `active_thread_id` valve/param in `smolcrawl/deep_research_tool.py`; no thread ⇒ unthreaded inbox (session only)
- [ ] **3.3** Preserve synthesis upsert (`uq_sources_synthesis_key`) + volatility/revalidate (additive now *requires* 3.0)
...
- [ ] **5.1** `OB1/integrations/suggestion-worker/index.ts` (model on `entity-extraction-worker`); reuse existing `source_extraction_queue` or sibling `suggestion_queue` (C4); `match_sources` cosine vs other threads → `thread_sources(suggested,pending,reason)`
- [ ] **5.2** Suggested-only rule — never auto-confirm cross-thread (concept §4.2)
- [ ] **5.3** `SUGGESTION_THRESHOLD` env + dedup vs existing links (any status); HTTP drain route `POST /suggest` style (not `/run`, C4)
- [ ] **5.4** Note three-places sync owed in Phase 8.2 (new container)
...
- [ ] **7.1** ON "send session to Obsidian inbox" → markdown stub into wiki `notes/` (summary + source refs + claims; deliberately incomplete)
- [ ] **7.2** Respect separation: stubs in `notes/` not `content/`; not ingested as a `source`
- [ ] **7.3** Sandbox targets a scratch clone — never push to real `openbrain-wiki-data` remote
...
- [ ] **8.1** Run §6 scenarios end-to-end in `iks-dev` → `iks-dev/e2e-results.md`
- [ ] **8.2** Three-places sync for `suggestion-worker`: OB1 compose + `scripts/emergency-recovery.ps1` + `.bat` (inventory + ordered start/stop) + `.claude/skills/stack-map/references/workspace-stacks.md`; `/stack-map` shows no drift vs baseline
- [ ] **8.3** `PROMOTION-RUNBOOK.md` (operator-executed): backup first → migration order (`init-threads.sql` → `content_hash` → `migrate-on-sources` dry-run then real) → rebuild order → rollback → per-repo commit checklist (D4)
- [ ] **8.4** Verify `openbrain-db` backup covers new tables (don't assume)
