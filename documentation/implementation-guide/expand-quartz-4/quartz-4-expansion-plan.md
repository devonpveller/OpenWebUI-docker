...
> - **Remaining (needs the live stack):** ~8 more Quartz components (`NotebookIndex`/`NotebookPage.inline`/`MembershipPicker`/`NotesEditor`/`SourceEditor`/`SourceLinker`/`ImportStatus`/leaf template + `ProvenancePanel`); the `## Evolution` derivation (P6.7) + notebook `graph.json` nodes (P2.6 polish); X.2 backup-job wiring; and running every phase's **gate** + the migrations against the running stack.
>
