...
(`frontend/docker-compose.yml:167`) and confirm it is found before trusting a
clean result.
...
|---|---|---|
| `frontend` -> `inference` | `frontend/docker-compose.yml:161`, `:163`, `:164`; also `entrypoint.sh:70`, `:72` (`LITELLM_UI_HOST` -> `llm-gateway-ui`, enabled true) consumed at `entrypoint.sh:103` | `LLAMA_CPP_HOST` defaults to the alias but sits behind `LLAMA_CPP_ENABLED` (`:163`); OWUI serves without it |
| `frontend` -> `search` | `frontend/docker-compose.yml:102` | `SEARXNG_QUERY_URL` default `http://gateway:8080/search`; web search off, chat unaffected |
| **`frontend` -> `ob1`** *(added attempt 2)* | `frontend/docker-compose.yml:167` (`OPEN_NOTEBOOK_HOST=${...:-open_notebook}`), `:169` (`OPEN_NOTEBOOK_ENABLED` default **true**), ports `:168`, `:171`; consumed by `entrypoint.sh:97`, `:99`. Also `entrypoint.sh:66` - the image's own `QUARTZ_HOST` fallback is `openbrain-wiki-viewer` | `open_notebook` is an ob1-plane service (`OB1/docker/docker-compose.yml:1112`); with ob1 down the two tailnet serve routes are dead, OWUI itself is not |
| **`frontend` -> `portal`** *(added attempt 2)* | `frontend/docker-compose.yml:183` (`QUARTZ_HOST=${...:-caddy}`), `:185` (`QUARTZ_ENABLED` default **true**), comment `:173-182`; consumed by `entrypoint.sh:101` | `caddy` is a portal-plane service (`portal/docker-compose.yml:169`). Note this is the reverse of `portal -> frontend` above: the portal hard-needs openwebui for its main vhost, the frontend's tailnet wiki route softly needs caddy |
