...
| 40 | same | "eight of them" — every OB1 compose invocation in the file uses the one list | `grep -c 'docker compose -f $Script:OB1Compose @prof' scripts/recovery/emergency-recovery.ps1` -> **8**. Then `grep -n 'docker compose -f $Script:OB1Compose' … \| grep -v '@prof'` -> exactly **one** line, and it is the COMMENT at the `$Script:OB1Profiles` declaration (`# ONE profile list for EVERY ...`). Any second line is an invocation that drifted and is a FAIL |
