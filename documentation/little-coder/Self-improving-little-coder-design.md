...
- **`git-proxy`** — wraps every git call. Whitelist: `commit`, `branch`, `checkout`, `merge --no-ff`, `tag`, `revert`, `reset --hard <tag>`, `fetch` (restricted to remotes pre-configured by the operator; no all-refs, no new remotes mid-task — an unconstrained `fetch` against a hostile remote can pollute every worktree off the canonical clone, §17). Blocklist: `push --force`, `branch -D`, `filter-branch`, `gc --prune=now`, `remote add`, `remote set-url`, all `submodule` subcommands (no legitimate agent need to init/update submodules; submodule URLs in `.gitmodules` are a network egress vector), history rewrites, anything touching `.git/` directly. **Sited at the open-terminal workspace edge** (§17): git inside the workspace is the proxied binary, and little-coder has no un-proxied raw-git path it can issue into the terminal. The whitelist's guarantees hold only if it is the _only_ git path. **`.git/` is part of the trust boundary, not just the git binary.** Blocking `remote add` at the command level is bypassable by writing the entry directly into `.git/config` (a normal file write, not a git call) and then issuing a "legitimate" `fetch` against the smuggled remote. The proxy alone cannot stop this — and the same vulnerability class extends past `.git/config` to several other files git executes or trusts. **Enforcement covers the whole class:**

- **`.git/config`, `.git/hooks/`, `.git/info/` mounted read-only to the agent.** Allowed remotes are baked in by the operator at project-switch time (§17). The agent literally cannot edit git config, install hooks (`.git/hooks/pre-commit` etc. — direct code-execution vector on whitelisted `commit`/`checkout`), or edit `info/exclude`. Git's runtime writes go to dynamic paths (`.git/objects/`, `.git/refs/`, `.git/index`, `.git/worktrees/<id>/`) which stay writable; the static-policy paths are pinned.
- **`.gitattributes` filter-driver execution is neutralized at the config layer.** A writable `.gitattributes` can declare `filter=foo`, but `foo` is undefined unless declared in the (read-only) `.git/config`, so the directive is a no-op. Closing `.git/config` closes `.gitattributes` for free.
- **`.gitmodules` is closed at the proxy** (blocklist above). A writable `.gitmodules` cannot cause submodule clones because no `submodule` subcommand is whitelisted.
- **Belt-and-suspenders for hooks:** the operator-baked `.git/config` sets `core.hooksPath` to an operator-controlled directory outside `.git/`. Even if the `.git/hooks/` read-only mount were somehow defeated, hook resolution wouldn't go there.

The agent has no legitimate need to edit any of these, so closed-by-default is the right posture. A future flexibility option (`.git/config` watched and validated at proxy time, allowing safe non-remote edits) is noted in §15 but not built. **Trust boundary:** operator-level git (via `docker exec`) bypasses the proxy by design — agent untrusted, operator trusted; the proxy exists for the agent, not the operator.
...
6. On pass: tear down the candidate and its staged workspace; little-coder opens a **pull request** from `auto/<date>-<topic>` to `main` on the **private** self-improvement remote (§12). The PR body is mechanically templated from the cohort and validation records and carries:
   ...
   **The repro itself is persisted, not just described.** Before candidate teardown, the executable form of the §8 cluster's repro case (input files, commands, expected output) is written to `little-coder-cohorts/repro/<artifact_id>/` — a path on the surviving named volume from §16 step 1. Step 7's smoke task locates and re-runs it against the deployed-active container after the rebuild. Without this persistence the smoke task has nothing to execute; the candidate workspace it was generated in no longer exists.
...
**Floor: a low-cadence spot-check, regardless of the drift signal.** The drift trigger catches _behavior change_; it does not catch a filter that was wrong from day one against a category that never showed up in the original test set — the rejection rate stays stable and silent. A small uniform sample, **quarterly, audited by hand**, exists as a floor under the drift trigger. The drift trigger remains the primary mechanism; the floor catches "we never knew to look for this." Tighten the floor cadence if a floor audit ever surfaces a class the drift trigger missed.
...
**Single global lock on the open-terminal workspace.** The trivially-correct concurrency model. Rejected because it serializes the inner loop behind the outer loop (validation blocks all interactive work, and vice versa), violating §3's separation. Replaced by per-session-id serial + per-repo serial + bounded validation parallelism (§17).
...
**Combined `agent` + `meta` in one process.** Tempting (fewer moving parts, simpler IPC). Rejected to preserve §3 loop separation at the _process_ level: a meta-loop bug — clustering taking too long, a judge call hanging, validation hung on Polyglot — would deadlock the inner loop responding to user tasks. Two processes share state via the persistent stores (§19/§20/§22); their failure domains stay separate.

**Cluster IDs derived from embedding hash.** Considered for "naturally stable, no synthetic id needed" identity. Rejected because the ids would be tied to the embedding model version — and §13's oracle DIP note implies the embedding/scoring stack is expected to evolve. An embedding-model swap would invalidate every `cluster_id` and lose the entire cohort history. Replaced by synthetic immutable `cluster_id`s decoupled from any model (§20).

**Synchronous Polyglot validation inside the inner loop.** Considered for tight feedback: every fix attempt validated immediately. Rejected for cost/latency — the inner loop responds to tool errors in seconds, a Polyglot subset takes minutes. The two-track model (§9 acute + longitudinal, §13 validation at merge time) is the chosen alternative; Polyglot runs at outer-loop cadence where its cost is amortized over many inner-loop tasks.
