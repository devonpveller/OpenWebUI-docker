...
> **Source of truth for design:** [Self-improving-little-coder-design.md](Self-improving-little-coder-design.md). This plan is the build sequencing; the design doc is the *why* and the *what*. If the two disagree, the design doc wins and this plan gets updated.
>
...
- Multi-project concurrency in one open-terminal (one focused project at a time, see `§17`).
...
| Workspace + exec     | `open-terminal` (existing container)                | Repo lives here, edits + tests + commands run here. Per-task git worktrees off a canonical clone.               |
| Inference            | `llama-cpp` / llama-swap (existing)                 | `qwen3.6:27b` (reasoning) + `qwen3.6:27b-nothink` (fast). Both little-coder and the judge are clients.          |
...
| 2    | Routing rule (config file)                       | Same persistence after tier-1              | Decision-shape  | Yes, once trusted         | Usually no; per-rule verify       |
...
**Goal:** lock the prerequisites that, if missed, are unrecoverable later. Make the open-terminal network posture compatible with hosting an autonomous loop.

- Confirm inference reachability: `http://llama-cpp:8080/v1` for both `qwen3.6:27b` and `qwen3.6:27b-nothink`.
- Inventory current `open-terminal` compose block (network_mode, capabilities, mounts). Identify the delta to the target posture in `§17` ("own network with explicit egress to `llama-cpp` + operator-configured git remote + private search gateway, nothing else").
...
**Exit:** gap list closed; open-terminal compose change scoped (does not need to ship in this phase, but the delta is known and tracked in `§15`); inference + remote + PAT verified reachable from a throwaway container on the right networks.
...
**Exit:** a Flow 2 (CLI) trigger completes end to end against a throwaway repo, every line of `tool_calls.jsonl`/`errors.jsonl`/`outcomes.jsonl` validates against the envelope schema, the trio of journals survives a container `up -d --build` without data loss, healthchecks green.

### Phase 2 — Workspace edge + CLI operator surface + preflight start

**Goal:** make open-terminal the workspace (per `§17`), stand up the operator surface (CLI first, OWUI later), and start collecting real journals.

- Implement the `§17` session model in `agent`: per-task git worktree under `/workspace/sessions/<session_id>/` off a canonical clone in the volume. Per-session serial; per-repo serial; FIFO queue across triggers (no "busy, try later").
- Implement project focus: `/project repo: <link>` CLI subcommand. URL normalization (host+owner+repo, lowercased). Reject mid-flight if active sessions or active validation worktrees exist. Journal `project_switched`.
...
- Begin preflight (`§14`): journals on, meta-loop **off**. Real OWUI/CLI users driving real workloads against open-terminal. Run for ≥ 1–2 weeks.
...
**Goal:** the agent has no un-proxied raw-git path. The whitelist's guarantees only hold if it is the *only* git path.
...
- Mount `.git/config`, `.git/hooks/`, `.git/info/` **read-only to the agent**. Allowed remotes baked in by operator at project-switch time. Dynamic paths (`.git/objects/`, `.git/refs/`, `.git/index`, `.git/worktrees/<id>/`) stay writable.
...
- Confirm `§24` preflight exit criteria, all three required:
  - (a) ≥ K distinct clusters each have ≥ their M window of *observed* occurrences;
...
- Build tier-1: judge picks tool-craft *vs* plan-slot within the tier; choice journaled. Selection prompt per `§5`.
...
**Exit:** at least one tier-1 artifact merged with cohort improvement; tier-0 auto-merge running for ≥ M tasks without efficacy reversion of more than X% of merged artifacts (X tuned in preflight).
...
**Exit:** one merged routing rule with measurable cohort improvement *and* exploration evidence that the rule is correct.
...
| 2 | Session model: per-session git worktrees off a canonical clone of one focused project. Per-session and per-repo serial; bounded-N validation parallelism; tier-3 escalates to ephemeral container. | `§17`, `§15` |
...
| 5 | Open-terminal network posture (ai-stack compose change) | ai-stack-level work outside this doc | Phase 6 hardens; Phase 1 functional without it |
| 6 | Sanitization audit drift threshold                     | Preflight rejection-rate baseline | Phase 4 (`meta`-on) |
| 7 | Reserved-slot promotion threshold (`meta` GPU)         | Preflight + steady-state load | Phase 8 (only if starvation observed) |
| 8 | Backup cadence + restore drill                         | Decided alongside volumes      | Phase 1 (volumes); drill before Phase 8 (tier-1) |
| 9 | `.git/config` flexibility upgrade                      | Deferred; only if real need arises | None today |
...
- **Evidence-triggered, not time-based.** Cohort math, efficacy reversion, sanitization audits, escalation. The only fixed-cadence thing in the design is the quarterly sanitization floor (`§23`).
...
| `§17` Service surface       | 1, 2, 6  | open-terminal as workspace; session/project/persistence model |
...
- **Update both together.** If a phase exit criterion changes, change it here *and* in the tasks doc; if a decision flips, record it in the tasks decision log *and* update this plan's lock table.
