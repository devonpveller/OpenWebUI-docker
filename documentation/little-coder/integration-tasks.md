...
- **Active chapter:** 3 (Observer) — build-complete and deployed 2026-05-23: cluster data model + cohort projection + meta iteration runner + judge (LLM-in-the-loop) + operator surface (`/admin/observe`, `lc admin observe`, OWUI `/observe`). `observer.enabled: true` is live; first iteration walked 106 accumulated journal records and surfaced 4 unassigned occurrences. Awaiting accumulated cluster-eligible volume + the operator's judge-prompt dry-run (open item #2) to validate calibration before the judge is auto-wired into the daemon's MetaRunner. Chapter 2 (OWUI pipeline) closed 2026-05-23; chapter 1 (Tool) operator-accepted 2026-05-22.
