...
- **Active chapter:** 3 (Observer) — **build-complete and deployed end-to-end 2026-05-23**: cluster data model + cohort projection + meta iteration runner + judge (LLM-in-the-loop) + judge auto-wire + evidence-triggered auto-iteration on task-end + Observer Prometheus metrics + operator surface (`/admin/observe`, `lc admin observe`, OWUI `/observe`). Deployed config: `observer.enabled: true`, `auto_iterate_on_task_end: true`, `judge_enabled: false` (operator-gated pending the prompt-calibration dry-run, open item #2). First iteration walked 112 accumulated journal records; 4 unassigned occurrences surfaced. **Remaining work for chapter-3 → chapter-4 is operator-only** (dry-run, judge enable, run real tasks, decide whether the minted labels are trustworthy). Chapter 2 closed 2026-05-23; chapter 1 operator-accepted 2026-05-22.
