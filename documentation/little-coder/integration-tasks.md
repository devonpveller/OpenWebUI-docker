...
- **Active chapter:** 5 (Self-modifier) — Stage 1 (§5a tier-0 auto-merge) landed 2026-05-23. Config flag + sample-fraction + deterministic sampling; 417 tests passing. Stages 2+ (tier-2 routing rules, tier-3 §6 justification gate, tier-3 candidate topology, tier-3 PR/deploy/rollback, upstream pull) remain — these are the biggest builds in the project, each gated by real operator-driven Chapter-4 evidence accumulating first. Chapter 4 build closed with tier-0 AND tier-1 paths; what's NOT in Chapter 4 (deferred but tracked): Polyglot corpus import (operator), planner-process plan-slot hot-reload (upstream integration), per-window budget rate-limits (operational), trend-miner rotation hook (alarm-routing follow-up). Chapter 3 (Observer) build-complete + deployed; judge auto-wire still gated on operator dry-run (open item #2). Chapter 2 closed 2026-05-23; chapter 1 operator-accepted 2026-05-22.
