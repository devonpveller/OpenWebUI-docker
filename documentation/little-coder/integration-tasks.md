...
- **Active chapter:** 4 (Learner) — **build-complete for the tier-0 path** as of 2026-05-23. Stages 1–7 landed: skills data model + augmenter + tier-0 escalation + judge drafting + validation gates + efficacy reversion + Polyglot oracle interface + operator approve/reject + longitudinal track. 396 tests passing. Operator approve/reject surface verified live (planted a draft, approved it, rejected it, audit log shows both rows). What's NOT in Chapter 4: tier-1 logic (rate-delta math + judge tool-vs-plan-slot picker — flagged for follow-up), §4f contradiction surface (Chapter 5 supersession lineage), real Polyglot corpus import (operator action). Chapter 3 (Observer) build-complete + deployed; judge auto-wire still gated on operator dry-run (open item #2). Chapter 2 closed 2026-05-23; chapter 1 operator-accepted 2026-05-22.
...
- [~] Surface efficacy-reversion notices (design §8.5) — `efficacy.retire_ineffective_skills` records `RetirementDecision`s; the OPERATOR-VISIBLE rendering on `/admin/pending` is partial (rejected skills show in audit log but the surface doesn't yet pull a retirement-notice feed). Real wiring depends on Stage-6-follow-up that snapshots `(pre_count, pre_window_tasks)` at approve time
...
- [~] Trend miner runs on rotation events (design §4.3) — `detect_anomalies(history, window=5, tolerance=0.25)` is the trend miner; rotation-event integration (auto-run on journal rotation) is operator-triggered for now (Stage-7 ships the function; the meta-loop hook is a small follow-up)
