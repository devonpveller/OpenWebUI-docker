...
| plan ready, before any dispatch | `gate-plan N` | the FIRST of the two gates (operator design 2026-08-22): machine pre-checks (verdict `fix` + repro `confirmed-in-code` + fresh base) then an independent GO/NO-GO review of the plan itself. GO unblocks dispatch to the worker org; NO-GO goes back to the planner. Exit 0=GO, 2=NO-GO(review), 1=NO-GO(machine) |
| a worker PR appears | `gate <PR#>` | the SECOND gate: post the verdict (RECOMMEND-MERGE / DENY + orchestration-adjustment plan) to the thread; the operator merges, never you. Order on a PR: static gate FIRST, T2 twin only after the diff reads clean — T2 executes the branch's code near live services |
