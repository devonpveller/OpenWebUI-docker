...
| `verify-commit-path-guard.ps1` | Executable proof of `.githooks/reference-transaction`, the **commit-path guard** (U5). The **commit-path drill runs 66 checks**: that `--no-verify` on commit/amend/merge cannot advance a branch — including the retry after `commit-msg` refuses a message, and a message-only `--amend --no-verify` — that honest work (partial commit, clean merge, conflict resolution, reviewer rebase, cherry-pick, reset, ff pull) is untouched, a **negative control** showing the same bypass succeeds once the hook is removed, and a **measured boundary table** executing all six routes that survive. Safe to run concurrently with itself (run-unique scratch root, touches no plane and no container, needs no lease). `-Audit` prints this machine's refusal log |
