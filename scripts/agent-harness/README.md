...
| `verify-commit-path-guard.ps1` | Executable proof of `.githooks/reference-transaction`, the **commit-path guard** (U5): 33 checks that `--no-verify` on commit/amend/merge cannot advance a branch, that honest work (clean merge, conflict resolution, reviewer rebase, cherry-pick, reset, ff pull) is untouched, and a **negative control** showing the same bypass succeeds once the hook is removed. `-Audit` prints this machine's refusal log |
