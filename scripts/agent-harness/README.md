...
| `verify-commit-path-guard.ps1` | Executable proof of `.githooks/reference-transaction`, the **commit-path guard** (U5). The **commit-path drill runs 113 checks** (three to four minutes — 185-239 s across the timed runs on 2026-08-30; it builds five scratch repositories, one a real submodule of another): that `--no-verify` on commit/amend/merge cannot advance a branch — including the retry after `commit-msg` refuses a message, a message-only `--amend --no-verify`, and every shape whose text the old message *reduction* discarded (a `#` line, a scissors block, blank runs, CRLF) — that honest work (partial commit, clean merge, conflict resolution, reviewer rebase, cherry-pick, reset, ff pull) is untouched, a **negative control** showing the same bypass succeeds once the hook is removed, a **measured boundary table** executing all six routes that survive, and two completeness steps: the canonical message form is a **fixed point** of every way git stores a message, and every tracked file that reads the attestation ledger is **registered with a role** with every registered verifier proved by a behavioural check in the same run. Safe to run concurrently with itself (run-unique scratch root, touches no plane and no container, needs no lease). `-Audit` prints this machine's refusal log |
