...
| `verify-reap.ps1` | Executable proof for `reap.ps1`: 9 cases, each building the thing that must SURVIVE next to the thing that must go. Self-cleaning; `-Script <path>` drives a modified copy so you can watch it go red. Fixtures are all named `reapv-<pid>-*`; most carry the ownership label, and three deliberately do not because the orphan and in-use cases need unlabelled resources to test at all |
