...
| `verify-merge-protocol.ps1` | Executable proof of MERGE-PROTOCOL's two-agent path, against a scratch line (never `development`), self-cleaning. SINGLE-FLIGHT since 2026-08-30: it takes the `merge-protocol-drill` lease first and exits 3 if another copy holds it, because it deletes and recreates FIXED branch and worktree names in the operator's checkout - two copies destroyed each other's runs and left the main checkout mid-rebase. `-LockProbe` takes that decision without running. Run it after changing any script here or the protocol |
| `test_drill_single_flight.py` | Repro + regression guard for the two defects that made the drill non-deterministic: the missing lease, and `git -C` ascending out of a half-provisioned worktree into the operator's checkout. Run it after touching the drill's preamble or `Test-IsWorktreeRoot` |
