...
| `reap.ps1` | CLEAN UP after the work: delete the docker containers and networks an owner labelled at creation. Called by `remove-worktree.ps1`; `-Report` on its own answers "what test junk exists and whose is it". The only script here whose ordinary job is deleting docker resources - `verify-reap.ps1` also deletes, but only fixtures it created in the same run - which is why its protection set (compose-managed, built-in networks) is checked positively and its reapable kinds are code, not config |
| `verify-reap.ps1` | the executable drill over the reaper - where a case is about what gets DELETED it builds the resource that must survive beside the one that must go; others take the daemon away, or read the live stack and build nothing |
