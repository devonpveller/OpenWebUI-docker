...
class Tools:
    ...
    def _resolve_path(self, subdir: str, filename: str, user_id: str = "") -> tuple:
        if self.valves.FILESHED_COMPATIBLE and user_id:
            ...
            base = os.path.join(self.valves.STORAGE_BASE_PATH, "superpowers", subdir)
    ...
    async def brainstorm(
        ...
        if not __messages__ or not any(
            "[SUPERPOWERS:MODE:"
            in (m.get("content", "") if isinstance(m, dict) else "")
        ...
        if __event_emitter__:
            await __event_emitter__(
                {
                    "type": "status",
                    "data": {"description": "Brainstorming...", "done": False},
                }
            )
    ...
    async def write_spec(
        ...
        if __event_emitter__:
            await __event_emitter__(
                {
                    "type": "status",
                    "data": {"description": "Writing spec...", "done": False},
                }
            )
        ...
        spec_path, storage_mode, zone_relative_path = self._resolve_path(
            self.valves.SPEC_DIR, filename, user_id
        )
...
Output ONLY the markdown document. No preamble, no commentary."""
        ...
        storage_label = (
            "Fileshed Storage zone (superpowers/)"
            if storage_mode == "fileshed"
            else "Standalone path"
        )

        if (
            self.valves.COMPLEXITY == "simple"
            and self._get_mode(__messages__) == "cook"
        ):
            if __event_emitter__:
                await __event_emitter__(
                    {"type": "status", "data": {"description": "Done.", "done": True}}
    ...
    async def review_spec(
        ...
        if __event_emitter__:
            await __event_emitter__(
                {
                    "type": "status",
                    "data": {"description": "Reviewing spec...", "done": False},
                }
            )
...
- [suggestions]"""
        ...
        resolved_path, _, zone_relative_path = self._resolve_path(
            self.valves.SPEC_DIR, os.path.basename(spec_path), user_id
        )
        read_path = (
            resolved_path
            if (self.valves.FILESHED_COMPATIBLE and zone_relative_path)
            else spec_path
        )
        try:
    ...
    async def write_plan(
        ...
        if __event_emitter__:
            await __event_emitter__(
                {
                    "type": "status",
                    "data": {"description": "Writing plan...", "done": False},
                }
            )
        user_id = (__user__ or {}).get("id", "")
        resolved_spec, _, spec_zone_path = self._resolve_path(
            self.valves.SPEC_DIR, os.path.basename(spec_path), user_id
        )
        read_spec_path = (
            resolved_spec
            if (self.valves.FILESHED_COMPATIBLE and spec_zone_path)
            else spec_path
        )
        try:
        ...
        user_id = (__user__ or {}).get("id", "") if __user__ else ""
        plan_path, storage_mode, zone_relative_path = self._resolve_path(
            self.valves.PLAN_DIR, filename, user_id
        )
    ...
    async def review_plan(
        ...
        if __event_emitter__:
            await __event_emitter__(
                {
                    "type": "status",
                    "data": {"description": "Reviewing plan...", "done": False},
                }
            )
...
- [optional suggestions, clearly marked as non-blocking]"""
        ...
        user_id = (__user__ or {}).get("id", "")
        resolved_path, _, zone_relative_path = self._resolve_path(
            self.valves.PLAN_DIR, os.path.basename(plan_path), user_id
        )
        read_path = (
            resolved_path
            if (self.valves.FILESHED_COMPATIBLE and zone_relative_path)
    ...
    def _validate_python(self, code: str, label: str, block_index: int) -> list:
        ...
        try:
            ...
            pyflakes_api.check(
                code, filename=f"<block{block_index}>", reporter=reporter
            )
    ...
    def _save_command_cache(self, cache: dict) -> None:
        ...
        try:
            self._atomic_write(
                self._get_command_cache_path(), json.dumps(cache, indent=2)
            )
        except Exception:
            pass
    ...
    def _detect_language(self, code: str) -> str:
        ...
        if re.search(
            ...
            return "powershell"
        if re.search(
            r"\bdef \w+\s*\(|\bimport \w|\bfrom \w+ import\b|if __name__", code
        ):
            return "python"
    ...
    def _save_lang_cache(self, lang: str, cache: dict) -> None:
        ...
        try:
            self._atomic_write(
                self._get_lang_cache_path(lang), json.dumps(cache, indent=2)
            )
        except Exception:
            pass
    ...
    def _get_knowledge_for_command(
        self, command: str, lang: str, source_int: int
    ) -> dict:
        ...
        for source_label, url in candidates:
            try:
                ...
                with urllib.request.urlopen(
                    req, timeout=self.valves.VALIDATION_TIMEOUT
                ) as resp:
            ...
            subcommands = list(
                set(
                    re.findall(
                        r"\b(put|get|cd|lcd|ls|pwd|bye|quit|mput|mget|mkdir|rmdir)\b",
                        text,
                    )
                )
            )
    ...
    def _fetch_from_url(self, command: str, url: str) -> dict:
        ...
        try:
            with urllib.request.urlopen(
                url, timeout=self.valves.VALIDATION_TIMEOUT
            ) as resp:
    ...
    def _update_cache_with_authority(
        self, cache: dict, command: str, new_data: dict
    ) -> bool:
        """
    ...
    def _validate_bash(self, code: str, label: str) -> list:
        ...
        if re.search(
            r"(REMOTE|DEST|TARGET|SSH).*PATH.*=.*\$\((whoami|hostname|pwd|id)\)", code
        ):
            issues.append(
    ...
    def _validate_javascript(self, code: str, label: str) -> list:
            ...
            try:
                ...
                result = subprocess.run(
                    [
                        ...
                        "--no-eslintrc",
                        "--rule",
                        '{"no-undef": "warn", "no-unused-vars": "warn"}',
                        tmp_path,
                    ],
                    capture_output=True,
                    text=True,
        ...
        if re.search(r"\.then\s*\([^)]*\)\s*(?!\.catch)", code):
            issues.append(
                f"{label}: .then() without .catch() — unhandled promise rejection"
            )
        if "eval(" in code:
    ...
    def _validate_generic_patterns(self, code: str, label: str, language: str) -> list:
        ...
        if re.search(r"\b(return|break|exit|sys\.exit)\b[^\n]*\n[ \t]+\S", code):
            issues.append(
                f"{label}: code appears after return/break/exit (unreachable)"
            )
        ...
        if re.search(r"while\s+[Tt]rue\s*[:{][\s\S]{0,50}break", code):
            issues.append(
                f"{label}: while-true with immediate break (loop never repeats)"
            )
    ...
    def _get_scratch_path(self, feature_name: str, user_id: str) -> str:
        path, _, _ = self._resolve_path("scratch", f"{feature_name}.scratch", user_id)
        return path
    ...
    async def execute_task(
        ...
        for line in lines:
            if line.strip().startswith(task_header):
                ...
                continue
            if in_task:
                if line.strip().startswith("### Task ") and not line.strip().startswith(
                    task_header
                ):
                    break
                task_lines.append(line)
    ...
    async def skillstack(
        ...
        if action == "-l":
            ...
            result["trust_level"] = trust_levels.get(
                result.get("source", "unknown"), 0.0
            )
            ...
            if was_updated:
                ...
                batch_active = (
                    self._batch_mode
                    if self._batch_mode is not None
                    else self.valves.BATCH_LEARN_MODE
                )
                if self.valves.AUTO_REVALIDATE_AFTER_LEARN and not batch_active:
                    ...
                    if code and command in code:
                        ...
                        if lang == "bash":
                            ...
                            issues = self._validate_powershell(
                                code, f"post-learn:{command}"
                            )
                        else:
                            issues = []
                        if issues:
        ...
        if action == "-r":
            ...
            result.setdefault(
                "trust_level", trust_levels.get(result.get("source", "unknown"), 0.0)
            )
        ...
        if action == "-s":
            ...
            cache_files = sorted(
                _glob.glob(os.path.join(cache_dir, "command_knowledge*.json"))
            )
            if not cache_files:
            ...
            for cache_file in cache_files:
                ...
                lang_label = (
                    "bash"
                    if fname == "command_knowledge.json"
                    else fname.replace("command_knowledge_", "").replace(".json", "")
                )
                try:
        ...
        if action == "-i":
            ...
            if valid_flags:
                ...
                lines.append(
                    "**Valid flags:** (none recorded — all flags pass validation)"
                )
            if valid_subcmds:
        ...
        if action == "-export":
            ...
            try:
                with open(
                    self._get_curated_kb_cache_path(), "r", encoding="utf-8"
                ) as f:
                    curated = json.load(f)
            except (OSError, json.JSONDecodeError):
                pass
            ...
            if self.valves.FILESHED_COMPATIBLE and user_id:
                ...
                export_dir = os.path.join(self.valves.STORAGE_BASE_PATH, "superpowers")
        ...
        if action == "-validate":
            ...
            if command:
                ...
                code_to_validate = self._extract_recent_code_from_context(
                    __messages__, user_id
                )
        ...
        if action == "-revalidate":
            ...
            if not code:
                return (
                    "No bash/shell code found in recent messages — nothing to validate."
                )
        ...
        if action == "-health":
            ...
            if not cache_files:
                status.append(
                    "✗ No knowledge cache files found — run `skillstack -init`"
                )
            else:
                for cache_file in cache_files:
                    fname = os.path.basename(cache_file)
                    lang_label = (
                        "bash"
                        if fname == "command_knowledge.json"
                        else fname.replace("command_knowledge_", "").replace(
                            ".json", ""
                        )
                    )
                    try:
                        ...
                        status.append(
                            f"✗ {lang_label} cache corrupted — run `skillstack -init` to reset"
                        )
        ...
        if action == "-init":
            ...
            if s["curated_created"]:
                ...
                lines.append(
                    f"✓ Curated KB: repaired from seed data ({kb_count} commands)"
                )
            else:
