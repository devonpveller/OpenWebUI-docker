...
| `search/gateway/`, `memory/mnemory-gateway/` (builds the `mnemory-cloud-gateway` container), `openbrain-gateway/`, `smolcrawl/`, `little-coder/` | Service source trees (the inference plane keeps its own in `inference/llm-queue/`) |
