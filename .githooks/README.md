...
| 5 | env_file scope | `scripts/checks/check-env-file-scope.ps1` | any `env_file:` in a staged compose file whose target resolves to the repo root `.env`, to another plane's directory, or outside the repo — whether or not HEAD already carried it. The pre-existing-grants exemption was removed on 2026-09-19 with the last two grants it was covering (`ao-worker-1`/`-2`); there is no per-service exemption. A plane's OWN `.env` beside or above its compose file is fine. Reads every value shape compose accepts (scalar, flow sequence, block sequence, long-form `path:`) and REFUSES one it cannot parse rather than guessing. `-All` audits the whole tree |
