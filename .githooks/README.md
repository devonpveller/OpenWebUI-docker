...
| 5 | env_file scope | `scripts/checks/check-env-file-scope.ps1` | any `env_file:` in a staged compose file whose target resolves to the repo root `.env`, to a directory that is neither the compose file's own nor a parent of it, or outside the repo — whether or not HEAD already carried it. The pre-existing-grants exemption was removed on 2026-09-19 with the last two grants it was covering (`ao-worker-1`/`-2`); there is no per-service exemption. What it checks is the PATH, read out of every value shape compose accepts (scalar, flow sequence, block sequence, long-form `path:`, and a YAML alias, which it follows to a `&name <scalar>` in the same file). What may reach the resolver is an ALLOWLIST — after stripping quotes and comments a path must be `^[A-Za-z0-9_./\\~-]+$` — and anything else (an unresolvable alias, an `&anchor`, a `${VAR}`, whitespace, flow residue) is REFUSED and printed, never normalized. `-All` audits the whole tree |
