...
| `openbrain-grounding-backfiller` | S2 brain-health worker; drains the `ungrounded_claims` view — per claim extracts its entity (local `:nothink` LLM) → fetches the Wikipedia page (via Tor) → `find_or_create_source` + `link_claim_to_source 'corroborates'` so confidence recomputes and the claim leaves the view; `POST /backfill?limit=N {thread_ids?}` (scoped to today's threads, or global sweep); deno-postgres | 127.0.0.1:8819 | obnet, llm-net, search-gw-net (Tor) |
