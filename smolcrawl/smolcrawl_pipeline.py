...
class Pipeline:
    ...
    def _submit_job(self, url: str, kb_name: str) -> str:
        ...
        if mode == "already_running":
            ...
            if live:
                ...
                progress = f" ({cursor}/{total} pages)" if total else ""
                return (
                    f"## SmolCrawl Pipeline\n\n"
                    f"⚠️ Job **`{live['id']}`** for _{kb_name}_ is already running{progress}.\n\n"
    ...
    def _stream_job(self, job: dict) -> Generator[str, None, None]:
        ...
        Not called from pipe() to avoid blocking the OWUI request thread.
        Kept for potential future use (e.g. a dedicated watch command).
        """
        q = job["queue"]
