...
description: >
  ...
  streams progress, and renders the GROUNDED synthesis. ALL the harness logic
...
class Tools:
    class Valves(BaseModel):
        ...
        poll_interval_sec: float = Field(default=2.0, description="How often to poll the job for progress.")
        max_wait_sec: int = Field(default=600, description="Give up waiting after this many seconds (the job keeps running server-side).")
        confidence_floor: float = Field(default=0.50, description="Reuse floor: claims below this are re-researched, not reused.")
    ...
    async def deep_research(
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                submit_body = {"query": query.strip(), "origin": "owui",
                               "options": {"confidence_floor": v.confidence_floor}}
                async with session.post(f"{base}/research", headers=headers, json=submit_body) as r:
                    if r.status == 401:
                ...
                if not job_id:
                    return f"Research engine returned no job id: {json.dumps(job)[:300]}"
                ...
                while waited < v.max_wait_sec:
                    ...
                    async with session.get(f"{base}/research/jobs/{job_id}", headers=headers) as r:
                        if r.status >= 400:
            ...
            return (f"Could not reach the research engine at {base} ({e}). "
